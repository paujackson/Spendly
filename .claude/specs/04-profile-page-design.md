...
- Step 02 — Registration (users must exist in the DB and must be creatable)
- Step 03 — Login and Logout (`session['user_id']` must be set on login; `/profile` must be a protected route)

## Routes
- `GET /profile` — render the profile page with user info and spending summary — logged-in only (redirect to `/login` if not authenticated)

## Database changes
No database changes. The `users` and `expenses` tables already have all required columns.

## Templates
- **Create:** `templates/profile.html` — profile page extending `base.html`; contains four sections:
  1. **User info card** — avatar initials, name, email, member-since date in format as "Month, Year" (all hardcoded)
